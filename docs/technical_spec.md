...
│  2. Context Layer     ← セッション文脈構築    │
│  3. Policy Engine     ← ルール評価           │
│  4. Enforcement Layer ← アクション実行       │
│       │                                     │
│       ├─ BLOCK  → エラーレスポンス返却       │
│       ├─ MASK   → マスク済みリクエスト転送    │
│       ├─ WARN   → ログ記録して転送           │
...
internal/
  ...
  context/
    builder.go              # リクエスト文脈構築
    session.go              # セッション管理（インメモリ）
...
### 4.1 設計方針
単発リクエストだけでなく、セッション内の累積的な文脈を考慮してリスク判定を行う。これがWAFとの最大の差別化ポイント。

### 4.2 Request Context

各リクエストから構築する即時コンテキスト。

```go
type RequestContext struct {
    RequestID   string    // リクエスト固有ID（UUID）
    Timestamp   time.Time
    Identity    Identity
    SessionID   string    // セッション識別子
    AppID       string    // アプリケーション識別子
    Model       string    // 要求されたモデル
    Messages    []Message // 入力メッセージ
    InputText   string    // 検査対象テキスト（全メッセージ結合）
    Detections  []Finding // Detector実行結果
}
```

### 4.3 Session Context

セッション内の累積情報。インメモリで管理し、TTLで自動削除する。
...
type SessionContext struct {
    ...
    BlockCount    int       // ブロック回数
    WarnCount     int       // 警告回数
    DetectionHist []string  // 検知履歴（detector名のリスト）
    RiskScore     float64   // 累積リスクスコア（0.0〜1.0）
}
```

### 4.4 累積リスクスコア

セッション内で繰り返し怪しい行動がある場合、リスクスコアを加算する。

```yaml
context:
  session:
    ttl: 30m            # セッションTTL
    store: memory       # memory（MVP）
  risk_scoring:
    injection_detected: 0.4
    pii_detected: 0.2
    confidential_detected: 0.3
    block_occurred: 0.3
    decay_per_minute: 0.01  # 時間経過で減衰
    threshold_warn: 0.5
    threshold_block: 0.8
```
  ...
  - name: session_risk_block
    ...
    when:
      session:
        risk_score_gte: 0.8
    action: block
    message: "セッションのリスクスコアが閾値を超えました。"

  - name: session_risk_warn
    phase: input
    when:
      session:
        risk_score_gte: 0.5
...
1. セッションベースポリシー（累積リスク）
2. 権限ベースポリシー（Identity × 検知結果）
3. 検知ベースポリシー（Detector結果のみ）
4. 最初にマッチしたBLOCKで即終了
5. BLOCKなしの場合、最も厳しいアクションを適用（BLOCK > MASK > WARN > ALLOW）
...
- 通知条件フィルタ + レートリミット
- エスカレーションルール
- セッション管理（インメモリ）+ 累積リスクスコア
- 偽陽性フィードバックAPI（`POST /v1/audit/{id}/feedback`）
...
**サイトロックアウト:** BLOCK判定時は3分間のサイトロックアウトを実施。黒背景でDOM全体を置換し、ロックアウト中はサイトの操作を不可にする。
...
|---|---|
| `allow` | そのまま送信を許可 |
| `block` | 送信を阻止、`message`をユーザーに表示、3分間サイトロックアウト |
...
テキスト検査:
  ...
  4. 結果に応じた処理:
     allow → 元の送信イベントを再発火
     block → 3分間サイトロックアウト（黒背景DOM置換）、送信を阻止
